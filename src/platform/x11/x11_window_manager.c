...
gf_x11_platform_create (void)
{
  gf_platform_interface_t* platform
      = gf_malloc (sizeof (gf_platform_interface_t));
  if (!platform)
    return NULL;
  ...
  platform->move_window_to_workspace
      = gf_x11_platform_move_window_to_workspace;
...
gf_x11_platform_init (gf_platform_interface_t* platform, gf_display_t* display)
{
  ...
  *display = XOpenDisplay (NULL);
  ...
  gf_x11_platform_data_t* data
      = (gf_x11_platform_data_t*) platform->platform_data;
...
gf_error_code_t
gf_x11_get_window_property (Display* display, Window window, Atom property,
...
gf_x11_window_has_state (Display* display, Window window, Atom state)
{
  ...
  if (gf_x11_get_window_property (display, window, atoms->net_wm_state,
				  XA_ATOM, &data, &nitems)
      != GF_SUCCESS)
    {
      return false;
...
gf_error_code_t
gf_x11_get_frame_extents (Display* display, Window window, int* left,
			  int* right, int* top, int* bottom)
{
  ...
  *left = *right = *top = *bottom = 0;
  ...
  gf_x11_atoms_t* atoms = gf_x11_atoms_get_global ();
  Atom sources[] = { atoms->gtk_frame_extents, atoms->qt_frame_extents,
  ...
  for (size_t i = 0; i < sizeof (sources) / sizeof (sources[0]); i++)
    {
      ...
      if (gf_x11_get_window_property (display, window, sources[i], XA_CARDINAL,
				      &data, &nitems)
	      == GF_SUCCESS
	  && nitems >= 4)
	{
			     ...
			     gf_window_info_t** windows, uint32_t* count)
{
  ...
  gf_window_info_t* filtered_windows
      = gf_malloc (nitems * sizeof (gf_window_info_t));
  if (!filtered_windows)
    {
      XFree (data);
  ...
  for (unsigned long i = 0; i < nitems; i++)
	...
	.geometry = geometry,
	.is_maximized
	= gf_x11_window_has_state (display, window,
  ...
  if (filtered_count == 0)
  ...
  else
    {
      ...
      *windows = gf_realloc (filtered_windows,
...
static gf_error_code_t
gf_x11_platform_set_window_geometry (gf_display_t display,
				     ...
				     gf_geometry_flags_t flags)
{
  ...
  XMoveResizeWindow (display, window, final_x, final_y, final_width,
		     final_height);
  XFlush (display);
					  ...
					  gf_workspace_id_t workspace_id)
{
  ...
  return gf_x11_send_client_message (display, window, atoms->net_wm_desktop,
				     data, 2);
}
				   ...
				   gf_native_window_t window)
{
  ...
  gf_x11_atoms_t* atoms = gf_x11_atoms_get_global ();
  long data[] = { 0, atoms->net_wm_state_maximized_horz,
		  atoms->net_wm_state_maximized_vert };

  return gf_x11_send_client_message (display, window, atoms->net_wm_state,
				     data, 3);
}

static gf_error_code_t
gf_x11_platform_get_window_geometry (gf_display_t display,
				     gf_native_window_t window,
				     gf_rect_t* geometry)
{
  if (!display || !geometry)
...
static bool
gf_x11_platform_is_window_valid (gf_display_t display,
				 gf_native_window_t window)
{
  if (!display)
    return false;
...
static bool
gf_x11_platform_is_window_excluded (gf_display_t display,
				    gf_native_window_t window)
{
  if (!display)
    return true;
  ...
  for (size_t i = 0;
       i < sizeof (excluded_states) / sizeof (excluded_states[0]); i++)
    {
  ...
  if (gf_x11_get_window_property (display, window, atoms->net_wm_window_type,
				  XA_ATOM, &data, &nitems)
      == GF_SUCCESS)
    {
	...
	{
	  for (size_t j = 0;
	       j < sizeof (excluded_types) / sizeof (excluded_types[0]); j++)
	    {
