...
static int gf_x11_error_handler(Display *display, XErrorEvent *error) {
  char error_text[256];
...
bool gf_x11_window_has_state(Display *display, Window window, Atom state) {
  ...
  if (gf_x11_get_window_property(display, window, atoms->net_wm_state, XA_ATOM,
                                 &data, &nitems) != GF_SUCCESS) {
    return false;
  }
...
gf_x11_platform_unmaximize_window(gf_display_t display,
  ...
  return gf_x11_send_client_message(display, window, atoms->net_wm_state, data,
                                    3);
}
...
gf_x11_platform_get_current_workspace(gf_display_t display) {
  ...
  if (gf_x11_get_window_property(display, root, atoms->net_current_desktop,
                                 XA_CARDINAL, &data, &nitems) == GF_SUCCESS &&
      nitems > 0) {
...
static bool gf_x11_platform_is_window_excluded(gf_display_t display,
  ...
  // Check window states
  Atom excluded_states[] = {atoms->net_wm_state_hidden,
                            atoms->net_wm_state_skip_taskbar,
                            atoms->net_wm_state_modal};

  for (size_t i = 0; i < sizeof(excluded_states) / sizeof(excluded_states[0]);
       i++) {
  ...
  if (gf_x11_get_window_property(display, window, atoms->net_wm_window_type,
                                 XA_ATOM, &data, &nitems) == GF_SUCCESS) {
