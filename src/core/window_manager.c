			  ...
			  gf_geometry_calculator_t* geometry_calc)
{
  ...
  *manager = gf_calloc (1, sizeof (gf_window_manager_t));
  ...
  gf_error_code_t result
      = gf_window_list_init (&(*manager)->state.windows, 16);
				      ...
				      gf_workspace_id_t workspace_id)
{
  ...
  gf_window_info_t* existing
      = gf_window_list_find (&manager->state.windows, window_id);

  gf_rect_t geometry;
  gf_error_code_t result = manager->platform->get_window_geometry (
      manager->display, window, &geometry);
  if (result != GF_SUCCESS)
    return result;

  gf_window_info_t window_info
      = { .id = window_id,
  ...
  for (uint32_t i = 0; i < manager->state.windows.count;)
    {
      ...
      if (!manager->platform->is_window_valid (manager->display,
...
gf_window_manager_print_stats (const gf_window_manager_t* manager)
{
  ...
  GF_LOG_DEBUG ("Window distribution (total: %u):",
		manager->state.windows.count);
				     ...
				     gf_workspace_id_t workspace_id)
{
  ...
  GF_LOG_DEBUG ("Arranging %u windows in workspace %d", window_count,
		workspace_id);
  ...
  result = manager->platform->get_screen_bounds (manager->display,
						 &workspace_bounds);
  ...
  manager->geometry_calc->calculate_layout (manager->geometry_calc, windows,
					    window_count, &workspace_bounds,
					    new_geometries);
...
gf_window_manager_run (gf_window_manager_t* manager)
{
  ...
  while (true)
	  ...
	  != manager->state.workspace_manager->active_workspace)
	{
      ...
      result
	  = gf_window_manager_arrange_workspace (manager, current_workspace);
      if (result != GF_SUCCESS)
	{
...
gf_window_list_ensure_capacity (gf_window_list_t* list,
...
gf_window_list_add (gf_window_list_t* list, const gf_window_info_t* window)
{
  ...
  gf_error_code_t result
      = gf_window_list_ensure_capacity (list, list->count + 1);
  if (result != GF_SUCCESS)
    return result;
  ...
  GF_LOG_DEBUG ("Added window %llu to workspace %d (total: %u)",
		(unsigned long long) window->id, window->workspace_id,
		list->count);
  return GF_SUCCESS;
}
  ...
  for (uint32_t i = 0; i < list->count; i++)
      ...
      if (list->items[i].id == window_id)
	{
	  ...
	  GF_LOG_DEBUG ("Removed window %llu from workspace %d (total: %u)",
			(unsigned long long) window_id, workspace_id,
			list->count);
	  return GF_SUCCESS;
	}
    }
...
void
gf_window_list_clear_update_flags (gf_window_list_t* list,
				   gf_workspace_id_t workspace_id)
{
  if (!list)
    return;
