...
gf_error_code_t
gf_workspace_manager_update (gf_workspace_manager_t* manager,
			     gf_display_t display)
{
  ...
  manager->active_workspace
      = manager->platform->get_current_workspace (display);
  ...
  for (uint32_t i = 0; i < workspace_count; i++)
      ...
      if (!gf_workspace_list_find (&manager->workspaces, i))
	{
	  ...
	  gf_error_code_t result = manager->platform->get_screen_bounds (
				      ...
				      gf_window_list_t* windows)
{
  ...
  for (uint32_t i = 0; i < overflow_count && free_count > 0; i++)
	...
	continue;
      ...
      for (uint32_t j = 0;
	   j < window_count && moved < excess && free_count > 0; j++)
	{
	  ...
	  if (result == GF_SUCCESS)
	    {
	      ...
	      uint32_t target_count = gf_window_list_count_by_workspace (
		  windows, target_workspace);
