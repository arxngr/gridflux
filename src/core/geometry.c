...
static void gf_bsp_split_recursive(const gf_window_info_t *windows,
                                   uint32_t count, const gf_rect_t *area,
                                   gf_rect_t *results, uint32_t padding,
                                   int depth) {
  if (count == 0)
    return;
  ...
  if (split_vertically) {
    ...
    gf_rect_t right_area = {area->x + left_width, area->y, right_width,
                            area->height};
    ...
    gf_rect_t bottom_area = {area->x, area->y + top_height, area->width,
                             bottom_height};
...
void gf_bsp_calculate_layout(const gf_geometry_calculator_t *calc_base,
                             const gf_window_info_t *windows, uint32_t count,
                             const gf_rect_t *workspace_bounds,
                             gf_rect_t *results) {
...
static void gf_grid_calculate_layout(const gf_geometry_calculator_t *calc_base,
                                     const gf_window_info_t *windows,
                                     uint32_t count,
                                     const gf_rect_t *workspace_bounds,
                                     gf_rect_t *results) {
...
void gf_rect_apply_padding(gf_rect_t *rect, uint32_t padding) {
  ...
  rect->x += padding;
  rect->y += padding;
  rect->width = (rect->width > padding * 2) ? rect->width - padding * 2
                                            : GF_MIN_WINDOW_SIZE;
  rect->height = (rect->height > padding * 2) ? rect->height - padding * 2
                                              : GF_MIN_WINDOW_SIZE;
}
