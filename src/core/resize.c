...
static bool
edge_line_from_dir (const gf_rect_t *r, gf_resize_dir_t dir,
                    int32_t *out_line, bool *out_horiz)
{
    if (dir == GF_RESIZE_BOTTOM)
        { *out_line = r->y + (int32_t)r->height; *out_horiz = true; }
    else if (dir == GF_RESIZE_TOP)
        { *out_line = r->y; *out_horiz = true; }
    else if (dir == GF_RESIZE_RIGHT)
        { *out_line = r->x + (int32_t)r->width; *out_horiz = false; }
    else if (dir == GF_RESIZE_LEFT)
        { *out_line = r->x; *out_horiz = false; }
    else
        return false;
    return true;
}
                         ...
                         int32_t search_range, int32_t *dists, gf_align_type_t *aligns)
{
    ...
    for (uint32_t i = 0; i < windows->count; i++)
    {
        ...
        int32_t w_max = is_horiz ? (wr->x + (int32_t)wr->width)
                                 : (wr->y + (int32_t)wr->height);
                      ...
                      bool is_horiz, bool *affected, int32_t *seg_min, int32_t *seg_max)
{
    ...
    while (changed)
    {
        ...
        for (uint32_t i = 0; i < windows->count; i++)
        {
            ...
            affected[i] = true;
            changed = true;
            if (w_min < *seg_min) *seg_min = w_min;
            if (w_max > *seg_max) *seg_max = w_max;
        }
    }
}

static uint32_t
collect_edge_neighbors (gf_win_list_t *windows, bool *affected,
                         gf_align_type_t *aligns, gf_segment_neighbor_t *out,
                         uint32_t max_out)
{
    uint32_t count = 0;
                         ...
                         gf_ws_id_t workspace_id, gf_monitor_id_t monitor_id)
{
    ...
    int32_t search_range = is_horiz ? (int32_t)initial->height * 2
                                    : (int32_t)initial->width * 2;
    ...
    expand_neighbor_set (windows, dists, min_dist, is_horiz, affected,
                         &seg_min, &seg_max);
...
static bool
corner_point_from_dir (const gf_rect_t *r, gf_resize_dir_t dir,
                        int32_t *out_cx, int32_t *out_cy)
{
    if ((dir & GF_RESIZE_RIGHT) && (dir & GF_RESIZE_BOTTOM))
        { *out_cx = r->x + (int32_t)r->width;  *out_cy = r->y + (int32_t)r->height; }
    else if ((dir & GF_RESIZE_LEFT) && (dir & GF_RESIZE_BOTTOM))
        { *out_cx = r->x;                       *out_cy = r->y + (int32_t)r->height; }
    else if ((dir & GF_RESIZE_RIGHT) && (dir & GF_RESIZE_TOP))
        { *out_cx = r->x + (int32_t)r->width;  *out_cy = r->y; }
    else if ((dir & GF_RESIZE_LEFT) && (dir & GF_RESIZE_TOP))
        { *out_cx = r->x;                       *out_cy = r->y; }
    else
        return false;
    return true;
}
                          ...
                          int32_t cx, int32_t cy)
{
    ...
    for (uint32_t i = 0; i < windows->count; i++)
    {
        ...
        const gf_rect_t *wr = &w->geometry;
        int32_t dx, dy, dsq;
        dx = wr->x - cx;                           dy = wr->y - cy;
        dsq = dx*dx + dy*dy; if (dsq < min_dist_sq) min_dist_sq = dsq;
        dx = wr->x + (int32_t)wr->width - cx;     dy = wr->y - cy;
        dsq = dx*dx + dy*dy; if (dsq < min_dist_sq) min_dist_sq = dsq;
        dx = wr->x - cx;                           dy = wr->y + (int32_t)wr->height - cy;
        dsq = dx*dx + dy*dy; if (dsq < min_dist_sq) min_dist_sq = dsq;
        dx = wr->x + (int32_t)wr->width - cx;     dy = wr->y + (int32_t)wr->height - cy;
        dsq = dx*dx + dy*dy; if (dsq < min_dist_sq) min_dist_sq = dsq;
    }
    return min_dist_sq;
}
...
collect_close_corners (gf_win_list_t *windows, gf_handle_t source_id,
                        gf_ws_id_t workspace_id, gf_monitor_id_t monitor_id,
                        int32_t cx, int32_t cy, int32_t min_dist_sq,
                        gf_corner_neighbor_t *out, uint32_t max_out)
{
    uint32_t count = 0;
    for (uint32_t i = 0; i < windows->count && count < max_out; i++)
    {
        ...
        dx = wr->x - cx; dy = wr->y - cy; dsq = dx*dx + dy*dy;
        corner = GF_CORNER_TOP_LEFT;
        if (dsq > min_dist_sq + 100)
        {
            dx = wr->x + (int32_t)wr->width - cx; dy = wr->y - cy;
            dsq = dx*dx + dy*dy; corner = GF_CORNER_TOP_RIGHT;
        }
        if (dsq > min_dist_sq + 100)
        {
            dx = wr->x - cx; dy = wr->y + (int32_t)wr->height - cy;
            dsq = dx*dx + dy*dy; corner = GF_CORNER_BOTTOM_LEFT;
        }
        if (dsq > min_dist_sq + 100)
        {
            dx = wr->x + (int32_t)wr->width - cx;
            dy = wr->y + (int32_t)wr->height - cy;
            dsq = dx*dx + dy*dy; corner = GF_CORNER_BOTTOM_RIGHT;
        }
        if (dsq <= min_dist_sq + 100)
        {
            out[count].win = w;
                            ...
                            gf_corner_neighbor_t *out_neighbors, uint32_t max_out)
{
    ...
    int32_t min_dist_sq = find_min_corner_dist_sq (windows, source_id, workspace_id,
                                                   monitor_id, cx, cy);
    int32_t radius_sq = (int32_t)(source_rect->width / 2) * (int32_t)(source_rect->width / 2);
    if (min_dist_sq > radius_sq)
        return 0;

    return collect_close_corners (windows, source_id, workspace_id, monitor_id,
                                  cx, cy, min_dist_sq, out_neighbors, max_out);
}
...
static void
clamp_source_edges (gf_win_list_t *windows, gf_win_info_t *source,
                    gf_resize_event_t *ev, uint32_t min_size,
                    int32_t *cx, int32_t *cy, int32_t *cw, int32_t *ch)
{
    if (ev->direction & GF_RESIZE_RIGHT)
        clamp_edge (windows, source, &ev->initial_rect, GF_RESIZE_RIGHT, min_size,
                    cx, cy, cw, ch);
    if (ev->direction & GF_RESIZE_LEFT)
        clamp_edge (windows, source, &ev->initial_rect, GF_RESIZE_LEFT, min_size,
                    cx, cy, cw, ch);
    if (ev->direction & GF_RESIZE_BOTTOM)
        clamp_edge (windows, source, &ev->initial_rect, GF_RESIZE_BOTTOM, min_size,
                    cx, cy, cw, ch);
    if (ev->direction & GF_RESIZE_TOP)
        clamp_edge (windows, source, &ev->initial_rect, GF_RESIZE_TOP, min_size,
                    cx, cy, cw, ch);

    clamp_all_corner_neighbors (windows, source->id, &ev->initial_rect, ev->direction,
                                 min_size, cx, cy, cw, ch,
                                 source->workspace_id, source->monitor_id);
}
...
enforce_source_min_size (gf_resize_event_t *ev, int32_t *cw, int32_t *ch,
                          uint32_t min_size)
{
    if (*cw < (int32_t)min_size) *cw = (int32_t)min_size;
    if (*ch < (int32_t)min_size) *ch = (int32_t)min_size;
...
apply_edges_to_neighbors (gf_win_list_t *windows, gf_win_info_t *source,
                           ...
                           gf_config_t *config)
{
    propagate_all_corner_neighbors (windows, source, &ev->initial_rect,
                                    &ev->current_rect, ev->direction, min_size,
