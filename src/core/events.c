...
gf_wm_watch (gf_wm_t *m)
{
    ...
    // We need to rewrite _handle_new_window to fetch the appropriate per-monitor target
    // workspace. Let's just pass NULL for current_ws for now so it derives it.
