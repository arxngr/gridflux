...
static void _on_window_dropdown_changed(GtkDropDown *dropdown, GParamSpec *pspec, gpointer data);
...
_handle_refresh_response(gpointer user_data)
{
    ...
    if (!ws_response || !win_response ||
        ws_response->status != GF_IPC_SUCCESS ||
...
_refresh_workspaces(gf_gtk_app_widget_t *app)
{
    ...
    GtkWidget *old = gtk_scrolled_window_get_child(
        GTK_SCROLLED_WINDOW(app->workspace_table));
    if (old)
...
_on_lock_clicked(GtkButton *btn, gpointer data)
{
    ...
    const char *ws_id = _get_selected_workspace_id(app->ws_dropdown,
...
_on_config_save_clicked(GtkButton *btn, gpointer data)
{
    ...
    GtkWidget *max_windows_spin = g_object_get_data(G_OBJECT(config_window), "max_windows_spin");
    GtkWidget *max_workspaces_spin = g_object_get_data(G_OBJECT(config_window), "max_workspaces_spin");
    GtkWidget *default_padding_spin = g_object_get_data(G_OBJECT(config_window), "default_padding_spin");
    GtkWidget *min_window_size_spin = g_object_get_data(G_OBJECT(config_window), "min_window_size_spin");

    config.max_windows_per_workspace = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(max_windows_spin));
    config.max_workspaces = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(max_workspaces_spin));
    config.default_padding = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(default_padding_spin));
    config.min_window_size = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(min_window_size_spin));
...
_on_config_button_clicked(GtkButton *btn, gpointer data)
{
    ...
    gtk_window_set_default_size(GTK_WINDOW(config_window), CONFIG_WINDOW_WIDTH, CONFIG_WINDOW_HEIGHT);
    ...
    _create_config_spin_row(form, config_window, "Max Windows per Workspace:",
                           "max_windows_spin", 1, 20, 1, config.max_windows_per_workspace);
    _create_config_spin_row(form, config_window, "Max Workspaces:",
                           "max_workspaces_spin", 1, 50, 1, config.max_workspaces);
    _create_config_spin_row(form, config_window, "Default Padding:",
                           "default_padding_spin", 0, 50, 1, config.default_padding);
    _create_config_spin_row(form, config_window, "Min Window Size:",
                           "min_window_size_spin", 50, 500, 10, config.min_window_size);
    ...
    g_signal_connect_swapped(close_btn, "clicked", G_CALLBACK(gtk_window_destroy), config_window);
...
_setup_window_icon(GtkWidget *window)
{
    ...
    GdkPixbuf *icon = gdk_pixbuf_new_from_file_at_scale(
        "icons/gridflux-48.png", 48, 48, FALSE, &error);
...
_create_control_panel(GtkWidget *main, gf_gtk_app_widget_t *widgets)
{
    ...
    gtk_box_append(GTK_BOX(controls), _create_button("🔒 Lock", G_CALLBACK(_on_lock_clicked), widgets));
    gtk_box_append(GTK_BOX(controls), _create_button("🔓 Unlock", G_CALLBACK(_on_unlock_clicked), widgets));
    gtk_box_append(GTK_BOX(controls), _create_button("🔄 Refresh", G_CALLBACK(_on_refresh_clicked), widgets));
    gtk_box_append(GTK_BOX(controls), _create_button("⚙️ Config", G_CALLBACK(_on_config_button_clicked), widgets));
}
...
_create_move_panel(GtkWidget *main, gf_gtk_app_widget_t *widgets)
{
    ...
    widgets->window_dropdown = _create_dropdown(&widgets->window_model, WINDOW_DROPDOWN_WIDTH);
    ...
    widgets->target_ws_dropdown = _create_dropdown(&widgets->target_ws_model, DROPDOWN_WIDTH);
    gtk_box_append(GTK_BOX(move_controls), widgets->target_ws_dropdown);

    GtkWidget *move_btn = _create_button("Move Window", G_CALLBACK(_on_move_clicked), widgets);
...
_gf_gtk_activate(GtkApplication *app, gpointer user_data)
{
    ...
    gtk_widget_set_size_request(widgets->workspace_table, TABLE_MIN_WIDTH, TABLE_MIN_HEIGHT);
