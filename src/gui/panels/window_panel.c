...
on_window_dropdown_changed (GtkDropDown *dropdown, GParamSpec *pspec, gpointer data)
{
    ...
    if (!window_data || selected >= window_data->len) return;
    ...
    for (uint32_t i = 0; i < workspaces->count; i++) {
        if (workspaces->items[i].id == current_workspace) continue;
        char ws_id_str[16]; snprintf (ws_id_str, sizeof(ws_id_str), "%d", workspaces->items[i].id);
...
on_move_clicked (GtkButton *btn, gpointer data)
{
    ...
    if (wsel == GTK_INVALID_LIST_POSITION || tsel == GTK_INVALID_LIST_POSITION) return;

    GListModel *model = gtk_drop_down_get_model (GTK_DROP_DOWN (app->window_dropdown));
    GArray *window_data = g_object_get_data (G_OBJECT (model), "window-data");
    if (!window_data || wsel >= window_data->len) return;

    gf_win_info_t *win = &g_array_index (window_data, gf_win_info_t, wsel);
    GtkStringObject *target_item = GTK_STRING_OBJECT (gtk_drop_down_get_selected_item (GTK_DROP_DOWN (app->target_ws_dropdown)));
    if (!target_item) return;

    char cmd[64];
    snprintf (cmd, sizeof cmd, "move %p %s", (void *)win->id, gtk_string_object_get_string (target_item));
...
gf_gui_window_panel_new (gf_app_state_t *app)
{
    ...
    g_signal_connect (app->window_dropdown, "notify::selected", G_CALLBACK (on_window_dropdown_changed), app);

    app->target_ws_model = gtk_string_list_new (NULL);
    app->target_ws_dropdown = gtk_drop_down_new (G_LIST_MODEL (app->target_ws_model), NULL);
