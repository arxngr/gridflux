    ...
    for (int t = 0; tries[t]; t++)
    {
        if (gtk_icon_theme_has_icon (theme, tries[t]))
        {
            GtkIconPaintable *icon
                = gtk_icon_theme_lookup_icon (theme, tries[t], NULL, 24, 1,
                                              GTK_TEXT_DIR_NONE, 0);
            if (icon)
...
refresh_rules_list (rules_panel_data_t *data)
{
    ...
    for (uint32_t i = 0; i < config.window_rules_count; i++)
    {
        ...
        // Workspace ID
        char ws_str[16];
        snprintf (ws_str, sizeof (ws_str), "%d",
                  config.window_rules[i].workspace_id);
        ...
        g_signal_connect (remove_btn, "clicked",
                          G_CALLBACK (on_remove_rule_clicked), NULL);
...
static void
setup_app_list_item (GtkSignalListItemFactory *factory, GtkListItem *list_item, gpointer data)
{
    (void)factory;
    (void)data;
...
static void
bind_app_list_item (GtkSignalListItemFactory *factory, GtkListItem *list_item, gpointer data)
{
    (void)factory;
    (void)data;
...
on_add_rule_clicked (GtkButton *btn, gpointer user_data)
{
    ...
    int workspace_id
        = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON (data->ws_spin));
...
on_rules_button_clicked (GtkButton *btn, gpointer data)
{
    ...
    g_signal_connect (rules_window, "destroy",
                      G_CALLBACK (on_rules_window_destroy), panel_data);
    ...
    gtk_drop_down_set_enable_search (
        GTK_DROP_DOWN (panel_data->app_dropdown), TRUE);
    ...
    g_signal_connect (add_btn, "clicked",
                      G_CALLBACK (on_add_rule_clicked), panel_data);
    ...
    g_signal_connect_swapped (close_btn, "clicked",
                              G_CALLBACK (gtk_window_destroy), rules_window);
