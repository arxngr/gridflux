...
{
    if (!win || !win->name[0]) return FALSE;
...
gf_build_workspace_grid (GtkGrid *grid, gf_ws_list_t *workspaces)
{
    ...
    const char *headers[] = {"Workspace", "Window Count", "Available Slots", "Layout", "Status"};
    for (int i = 0; i < 5; i++) {
...
gf_refresh_workspaces (gf_app_state_t *app)
{
    ...
    gf_ipc_response_t *win_ptr = g_object_get_data (G_OBJECT (app->window), "win_response");
    ...
    if (ws_resp.status != GF_IPC_SUCCESS || win_resp.status != GF_IPC_SUCCESS) return;
    ...
    for (uint32_t i = 0; i < workspaces->count; i++) {
        char id_str[16]; snprintf (id_str, sizeof(id_str), "%d", workspaces->items[i].id);
    ...
    for (uint32_t i = 0; i < windows->count; i++) {
        if (is_window_itself(&windows->items[i]) || !windows->items[i].is_valid) continue;
        gtk_string_list_append (new_window_model, windows->items[i].name);
        g_array_append_val (window_data, windows->items[i]);
    }

    g_object_set_data_full (G_OBJECT (new_window_model), "window-data", window_data, (GDestroyNotify)g_array_unref);

    gtk_drop_down_set_model (GTK_DROP_DOWN (app->ws_dropdown), G_LIST_MODEL (new_ws_model));
    app->ws_model = new_ws_model;
    gtk_drop_down_set_model (GTK_DROP_DOWN (app->window_dropdown), G_LIST_MODEL (new_window_model));
    app->window_model = new_window_model;
    gtk_drop_down_set_model (GTK_DROP_DOWN (app->target_ws_dropdown), G_LIST_MODEL (new_target_ws_model));
    ...
    gtk_scrolled_window_set_child (GTK_SCROLLED_WINDOW (app->workspace_table), GTK_WIDGET (grid));
