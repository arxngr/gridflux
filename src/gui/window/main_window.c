...
static const char k_app_css[] =
    ...
    " font-size: 14px; background: alpha(white,0.03); border: 1px solid alpha(white,0.09); }"
    ...
    ".gf-chip:hover { border-color: alpha(@gf_acc,0.50); background: alpha(@gf_acc,0.10); }"
    ...
    ".gf-tile.on { background: alpha(@gf_acc,0.50); border: 1px solid alpha(@gf_acc,0.55); }"
    ".gf-tile.max { min-width: 30px; min-height: 24px;"
    " background: alpha(#f6be00,0.40); border: 1px solid alpha(#f6be00,0.50); }"
    /* Pills + lock */
    ".gf-pill { border-radius: 999px; padding: 3px 11px; font-size: 11px; font-weight: bold; }"
    ".gf-pill.tiled { background: alpha(@gf_acc,0.16); color: #9ff0e4; }"
    ".gf-pill.max { background: alpha(#f6be00,0.16); color: #f6cf5a; }"
    ".gf-lock { border-radius: 999px; padding: 2px 10px; font-size: 11px; font-weight: bold;"
...
{
    gtk_window_set_titlebar (GTK_WINDOW (widgets->window),
                             gf_gui_toolbar_new (widgets));
